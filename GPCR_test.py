...
if __name__ == "__main__":
    ...
    """Load preprocessed data."""
    dir_input = ('dataset/' + 'GPCR_test' + '/word2vec_30/')
    smiles = np.load(dir_input + 'smiles.npy', allow_pickle=True)
    compounds = load_tensor(dir_input + 'compounds', torch.FloatTensor)
    proteins = load_tensor(dir_input + 'proteins', torch.FloatTensor)
    interactions = load_tensor(dir_input + 'interactions', torch.LongTensor)
    ...
    """Create a dataset and split it into train/dev/test."""
    dataset_test = list(zip(graphs, proteins2, interactions2)) 
    ...
    file_model = 'output/model/' + 'GPCR'
    AUCs = ('Epoch\tTime(sec)\tLoss_train\tAUC_test\tPRC_test\t'+
    ...
    for epoch in range(1, iteration+1):
        ...
        AUCs = [epoch, time, loss_train, AUC_test, PRC_test,
                precision_test,recall_test,F1_test,ACC_test]
